...
int my_read(const char *path, char *buf, size_t size, off_t offset, struct fuse_file_info *f){
	char buffer[BLOCK_SIZE_BYTES];
	int bytes2Read, totalRead = 0;
	//Cogemos el nodo
	NodeStruct *node = myFileSystem.nodes[f->fh];
	
	//No nos salimos del fichero
	if(offset > node->fileSize)
		return -1;
	
	//comprobamos cuantos bytes tenemos que leer	
	...
	while (totalRead < bytes2Read) {
		...
		if((lseek(myFileSystem.fdVirtualDisk, currentBlock * BLOCK_SIZE_BYTES + offBlock , SEEK_SET) == (off_t) - 1) ||
		        (read(myFileSystem.fdVirtualDisk, &buffer, BLOCK_SIZE_BYTES-offBlock) == -1)) {
			perror("Failed lseek/read in my_write");
			return -EIO;
		} 
...
static int my_unlink(const char *path) {
	int idxNode, idxFile;
	//comprpobamos si el archivo existe
	if((idxFile = findFileByName(&myFileSystem, (char *)path + 1)) == -1)
		return -ENOENT;
	//Calculamos el numero del nodo
	idxNode = myFileSystem.directory.files[idxFile].nodeIdx;
	//Truncamos el fichero con 0 para borrar el contenido
	if(resizeNode(idxNode,0) < 0)
		return -EIO;
	
	//Accedemos al archivo y lo marcamos como libre
	myFileSystem.directory.files[idxFile].freeFile = true;
	myFileSystem.directory.numFiles--;
	
	//Accedemos al nodo y lo marcamos como libre
	NodeStruct *node = myFileSystem.nodes[idxNode];
	node->freeNode = true;
	...
	//Actualizamos tabla(Mapa de bits) 
	updateBitmap(&myFileSystem);
	return 0;
}
