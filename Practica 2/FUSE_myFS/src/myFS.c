...
{	
	...
	node = malloc(sizeof(NodeStruct));

	if(lseek(myFileSystem->fdVirtualDisk, BLOCK_SIZE_BYTES * NODES_IDX, SEEK_SET) == (off_t) - 1) {
		perror("Failed lseek in readNode");
		return -1;
	}
	for(i = 0; i < MAX_NODES; i++) {
		
		if(read(myFileSystem->fdVirtualDisk, node, sizeof(NodeStruct)) == -1) {
			perror("Failed read in readInodes");
		return -1;
		}
		if (node->freeNode == 1)
			myFileSystem->nodes[i] = NULL;
		myFileSystem->nodes[i] = node;
	}
